...
# endif
    ...
    strncpy (CDFnameA, actualPathX, (size_t)ilen);
    CDFnameA[ilen] = '\0';
    ptr = strstrIgCase (actualPathY, ".cdf");
    ilen = (int) strlen (actualPathY) - strlen(ptr);
    strncpy (CDFnameB, actualPathY, (size_t)ilen);
    CDFnameB[ilen] = '\0';
    ...
    if (!sameFile) { 
      ...
      if (last == NULL)  strcpy(dstTmpPath, dstPath); 
        ...
        strncpy (dstTmpPath, dstPath, (size_t)ilen);
        *(dstTmpPath+ilen) = '\0';
      }
...
char *extension;	/* OUT: The extension in the path name */ 
{
  ...
  if (IsReg(pathT)) {
    strcpy (fullPath, pathT);
    strcpy (extension, ".cdf");
    return;
  }
