	   ...
	   if (Item->Var->min != NULL) {
	     if (STRINGdataType(Item->Var->dataType))
		   ...
		   (Item->Var->dataType == CDF_REAL4)) {
	           ...
	           EncodeValueFormat (Item->Var->dataType,
				      Item->Var->min, temp, NULL,
	           ...
	           EncodeValueFormat (Item->Var->dataType,
                 ...
                 if (!isnan(*(double *)Item->Var->min) &&
                     *(double *)Item->Var->min <= DEFAULT_EPOCH_PADVALUE) {
                   EncodeValueFormat (CDF_EPOCH, Item->Var->min, temp,
                                      NULL, 0, MAX_nonSTRING_VALUE_LEN, style,
                                      (size_t) sizeof(temp));
                 } else
	           EncodeValueFormat (CDF_REAL8, Item->Var->min, temp, "%.13e",
				      0, MAX_nonSTRING_VALUE_LEN, style,
				      (size_t) sizeof(temp));
               } else if (Item->Var->dataType == CDF_EPOCH16) {
                 if (!isnan(*(double *)Item->Var->min) &&
                     !isnan(*((double *)Item->Var->min)+1) &&
                     *(double *)Item->Var->min <= DEFAULT_EPOCH16_PADVALUE &&
                     *(((double *)Item->Var->min)+1) <= DEFAULT_EPOCH16_PADVALUE) {
                   EncodeValueFormat (CDF_EPOCH16, Item->Var->min, temp,
                                      NULL, 0, MAX_nonSTRING_VALUE_LEN, style,
                                      (size_t) sizeof(temp));
                 } else
	           EncodeValueFormat (CDF_REAL8, Item->Var->min, temp, "%.13e",
	     ...
	     if (STRINGdataType(Item->Var->dataType))
	       ...
	       char temp[MAX_nonSTRING_VALUE_LEN+1];
	           ...
	           EncodeValueFormat (Item->Var->dataType,
                 ...
                 if (!isnan(*(double *)Item->Var->max) &&
                     *(double *)Item->Var->max <= DEFAULT_EPOCH_PADVALUE) {
                   EncodeValueFormat (CDF_EPOCH, Item->Var->max, temp,
                                      NULL, 0, MAX_nonSTRING_VALUE_LEN, style,
                                      (size_t) sizeof(temp));
                 } else
	           EncodeValueFormat (CDF_REAL8,
				      Item->Var->max, temp, "%.13e",
				      0, MAX_nonSTRING_VALUE_LEN, style,
				      (size_t) sizeof(temp));
               } else if (Item->Var->dataType == CDF_EPOCH16) {
                 if (!isnan(*(double *)Item->Var->max) &&
                     !isnan(*(((double *)Item->Var->max)+1)) &&
                     *(double *)Item->Var->max <= DEFAULT_EPOCH16_PADVALUE &&
                     *(((double *)Item->Var->max)+1) <= DEFAULT_EPOCH16_PADVALUE) {
                   EncodeValueFormat (CDF_EPOCH16, Item->Var->max, temp,
                                      NULL, 0, MAX_nonSTRING_VALUE_LEN, style,
                                      (size_t) sizeof(temp));
                 } else
	           EncodeValueFormat (CDF_REAL8,
	     ...
	     if (STRINGdataType(Item->Var->dataType))
	       ...
	       char temp[MAX_nonSTRING_VALUE_LEN+1];
	           ...
	           EncodeValueFormat (BOO(EPOCHdataType(Item->Var->dataType),
                 ...
                 if (!isnan(*(double *)Item->Var->fill) &&
                     *(double *)Item->Var->fill <= DEFAULT_EPOCH_PADVALUE) {
                   EncodeValueFormat (CDF_EPOCH, Item->Var->fill, temp,
                                      NULL, 0, MAX_nonSTRING_VALUE_LEN, style,
                                      (size_t) sizeof(temp));
                 } else
	           EncodeValueFormat (CDF_REAL8, Item->Var->fill, temp,
				      "%.13e", 0, MAX_nonSTRING_VALUE_LEN,
                                      style, (size_t) sizeof(temp));
               } else if (Item->Var->dataType == CDF_EPOCH16) {
                 if (!isnan(*(double *)Item->Var->fill) &&
                     !isnan(*(((double *)Item->Var->fill)+1)) &&
                     *(double *)Item->Var->fill <= DEFAULT_EPOCH_PADVALUE &&
                     *(((double *)Item->Var->fill)+1) <= DEFAULT_EPOCH_PADVALUE) {
                   EncodeValueFormat (CDF_EPOCH16, Item->Var->fill, temp,
                                      NULL, 0, MAX_nonSTRING_VALUE_LEN, style,
                                      (size_t) sizeof(temp));
                 } else
	           EncodeValueFormat (CDF_REAL8, Item->Var->fill, temp,
...
Logical ToWalk () {
  ...
  int varCount = 0, itemN = 0, varX, searchType = 0; long maxRec = NO_RECORD;
  ...
  for (Item = exportHead, lineN = 2, varX = 0;
     ...
     if ((Item->Var->dataType == CDF_FLOAT) ||
       ...
       if (!isnan(*(double *)Item->Var->value) &&
           ...
           *(double *)Item->Var->value <= DEFAULT_EPOCH16_PADVALUE &&
           *(((double *)Item->Var->value)+1) <= DEFAULT_EPOCH16_PADVALUE) {
...
void **searchValue;
{
  ...
  ****************************************************************************/
  ...
  if ((Var->dataType == CDF_FLOAT) || (Var->dataType == CDF_REAL4)) {
    ...
    if (!isnan(*(double *)Var->value) &&
        *(double *)Var->value <= DEFAULT_EPOCH_PADVALUE) {
      EncodeValuesFormat (CDF_EPOCH, Var->numElems, Var->value, value,
		          NULL, 0, MAXvalueLEN, style,
		          (size_t) sizeof(value));
    } else
      EncodeValuesFormat (CDF_EPOCH, Var->numElems, Var->value, value,
    ...
    if (!isnan(*(double *)Var->value) &&
        !isnan(*(((double *)Var->value)+1)) &&
        *(double *)Var->value <= DEFAULT_EPOCH16_PADVALUE &&
        *(((double *)Var->value)+1) <= DEFAULT_EPOCH16_PADVALUE) {
      EncodeValuesFormat (CDF_EPOCH16, Var->numElems, Var->value, value,
		          NULL, 0, MAXvalueLEN, style,
		          (size_t) sizeof(value));
    } else
      EncodeValuesFormat (CDF_EPOCH16, Var->numElems, Var->value, value,
...
void **out;
{
  ...
  if (!IsaVirtualVariable(zVar, varName)) {
    ...
    status = CDFlib(GET_, BOO(zVar,zVAR_NUMDIMS_,rVARs_NUMDIMS_),
                               &dimensionality,
    ...
    status = CDFlib(SELECT_, BOO(zVar,zVAR_NAME_,rVAR_NAME_), component1,
                    GET_, BOO(zVar,zVAR_DATATYPE_,rVAR_DATATYPE_), &dataType,
                          BOO(zVar,zVAR_NUMELEMS_,rVAR_NUMELEMS_), &numElements,
                          BOO(zVar,zVAR_NUMDIMS_,rVARs_NUMDIMS_), 
                              &dimensionality,
                          BOO(zVar,zVAR_DIMSIZES_,rVARs_DIMSIZES_), dimSizes,
                          BOO(zVar,zVAR_MAXREC_,rVAR_MAXREC_), &maxRec,
                    NULL_);
